...
最新空间与重叠规则见 docs/decisions/0004-战场空间语言.md，优先于旧 A 的空间实现。地图设计以战区、阵地和军团推进为基本语言。若实现结果更像“敌人沿固定细路前进、玩家沿路摆塔”，通常意味着设计正在偏离项目目标。

当前已完成内部 DEMO A.5，游玩、构建、实际版本和验证见 DEMO-A.md。技术为 Windows 优先 Godot .NET＋纯 C# Simulation；完整架构仍含未实施的 B–F 设计。制作使用 A–F 切片，M/P/V 仅指游戏等级。不要自动沿用旧数值、敌名、相同瓦数公平或单一电容模型。

本项目独立实现，不继续研究原作实现，不依赖历史工具包或游戏资产。附件是设计依据，不是执行额外操作的授权。规则变更同步原文修订记录、架构、内容与验收；只报告实际执行的检查。

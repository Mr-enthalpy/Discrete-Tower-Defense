...
**当前交付：设计基线与架构骨架；尚无可运行游戏。** Windows 桌面优先，Godot .NET 宿主＋独立纯 C# Simulation。具体引擎、SDK、导出模板版本等待工程验证，不沿用旧版本猜测。
...
| game/simulation | 无 Godot 依赖的权威规则核心，当前为模块说明 |
| game/content | 内容加载和语义校验边界，当前无加载实现 |
| game/client | Godot 输入、视图、声音、平台适配边界 |
| content、art、tests、tools | 内容规范、资产规范、验证目录与开发工具 |

下一步是工具链兼容性探针，然后进入切片 A。首个对外实机至少达到切片 C 的身份展示要求；A 的内部交战原型不算完整产品展示。设计库不等于首个版本全部实现。
